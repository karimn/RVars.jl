Base.broadcastable(x::RVar) = x

# Custom broadcast style to ensure output is RVar
struct RVarStyle{N} <: Base.Broadcast.AbstractArrayStyle{N} end

function Base.BroadcastStyle(::Type{<:RVar{T, N}}) where {T, N}
    RVarStyle{N}()
end

function Base.BroadcastStyle(s1::RVarStyle{M}, s2::RVarStyle{N}) where {M, N}
    RVarStyle{max(M, N)}()
end

function Base.BroadcastStyle(s::RVarStyle{M}, ::Base.Broadcast.DefaultArrayStyle{N}) where {M, N}
    RVarStyle{max(M, N)}()
end

function Base.BroadcastStyle(::Base.Broadcast.DefaultArrayStyle{M}, s::RVarStyle{N}) where {M, N}
    RVarStyle{max(M, N)}()
end

function Base.BroadcastStyle(s::RVarStyle{M}, ::Base.Broadcast.Style{Tuple}) where {M}
    RVarStyle{M}()
end
...
function Base.similar(bc::Base.Broadcast.Broadcasted{RVarStyle{N}}, ::Type{T}) where {N, T}
    # A fused expression (x .* 2 .+ 1) nests Broadcasted objects inside bc.args, hiding the
    # RVar operands. Flatten first so the args are leaves and the scan below sees them.
...
function Base.copy(bc::Base.Broadcast.Broadcasted{RVarStyle{N}}) where {N}
    ...
    # below, and the RVar operands it hides would never be recycled or aligned.
    bcf = Base.Broadcast.flatten(bc)
    ...
    # scalar RVar per logical element. Each operand is reshaped so axis 1 is the
    # draws axis and axes 2.. are the logical shape — the layout ordinary broadcast
    # expansion already knows how to align:
    #   - a RVar contributes its store, recycled up to `max_nd` draws;
