...
- `types.jl` — the `RandomDraw` struct, its inner constructor/invariants, and the `AbstractArray` size/axes plumbing. `_broadcast_draws` (recycle a 1-draw array up to N draws) lives here.
...
- `abstractarray.jl` — indexing (`getindex` returns a scalar `RandomDraw` slicing across all draws), `setindex!`, `reshape`, `similar`, `copy`, and pretty `show` (mean ± sd).
